...
function toVal(out, key, val, opts) {
  ...
  out[key] = old == null
    ? nxt
    : (Array.isArray(old) ? old.concat(nxt) : [old, nxt]);
}
    ...
    if (option.isBoolean) {
      if (option.negated) {
        const hasStringTypeOption = options.some((o, i) => {
          return i !== index && o.names.some((name) =>
    ...
    if (showCommands) {
      ...
      sections.push({
        ...
        body: commands.map((command) => {
          return `  ${
            padRight(command.rawName, longestCommandName.length)
          }  ${command.description}`;
        }).join("\n"),
      });
      sections.push({
    ...
    if (options.length > 0) {
      ...
      sections.push({
        ...
        body: options.map((option) => {
          return `  ${
            padRight(option.rawName, longestOptionName.length)
          }  ${option.description} ${
...
${section.body}`
  ...
  checkOptionValue() {
    ...
    for (const option of options) {
      ...
      if (option.required) {
        const hasNegated = options.some((o) =>
          o.negated && o.names.includes(option.name)
        );
...
cli
  ...
  .action(async (root, options) => {
    ...
    // is ok here
    const { createServer } = await Promise.resolve().then(function () {
      return require("./chunks/dep-971d9e33.js");
    }).then(function (n) {
      return n.index$1;
    });
    try {
      ...
      if (global.__vite_start_time) {
        // @ts-ignore
        const startupDuration = perf_hooks.performance.now() -
      ...
      build.createLogger(options.logLevel).error(
        build.colors.red(`error when starting dev server:\n${e.stack}`),
        { error: e },
      );
      process.exit(1);
    }
...
cli
  ...
  .action(async (root, options) => {
    const { build: build$1 } = await Promise.resolve().then(function () {
      return require("./chunks/dep-971d9e33.js");
    }).then(function (n) {
      return n.build$1;
    });
    ...
    try {
      ...
      build.createLogger(options.logLevel).error(
        build.colors.red(`error during build:\n${e.stack}`),
        { error: e },
      );
      process.exit(1);
    }
...
cli
  ...
  .action(async (root, options) => {
    const { optimizeDeps } = await Promise.resolve().then(function () {
      return require("./chunks/dep-971d9e33.js");
    }).then(function (n) {
      return n.index;
    });
    try {
      ...
      build.createLogger(options.logLevel).error(
        build.colors.red(`error when optimizing deps:\n${e.stack}`),
        { error: e },
      );
      process.exit(1);
    }
...
cli
  ...
  .option("--open [path]", `[boolean | string] open browser on startup`)
  .action(async (root, options) => {
    const { preview } = await Promise.resolve().then(function () {
      return require("./chunks/dep-971d9e33.js");
    }).then(function (n) {
      return n.preview$1;
    });
    try {
      ...
      build.createLogger(options.logLevel).error(
        build.colors.red(`error when starting preview server:\n${e.stack}`),
        { error: e },
      );
      process.exit(1);
    }

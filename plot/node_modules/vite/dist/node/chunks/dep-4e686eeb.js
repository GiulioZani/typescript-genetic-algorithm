...
const TELEPORT = Symbol(
  (process.env.NODE_ENV !== "production") ? `Teleport` : ``,
);
...
const CAMELIZE = Symbol(
  (process.env.NODE_ENV !== "production") ? `camelize` : ``,
);
...
const WITH_CTX = Symbol(
  (process.env.NODE_ENV !== "production") ? `withCtx` : ``,
);
const UNREF = Symbol((process.env.NODE_ENV !== "production") ? `unref` : ``);
const IS_REF = Symbol((process.env.NODE_ENV !== "production") ? `isRef` : ``);
const WITH_MEMO = Symbol(
  (process.env.NODE_ENV !== "production") ? `withMemo` : ``,
);
...
function injectProp(node, prop, context) {
   ...
   */
  let props = node.type === 13 /* VNODE_CALL */
    ? node.props
    : node.arguments[2];
  let callPath = [];
  let parentCall;
...
function toValidAssetId(name, type) {
  ...
  return `_${type}_${
    name.replace(/[^\w]/g, (searchValue, replaceValue) => {
      return searchValue === "-"
        ? "_"
...
function parseElement(context, ancestors) {
  ...
  {
    const inlineTemplateProp = element.props.find((p) =>
...
function parseAttribute(context, nameSet) {
  ...
  if (!context.inVPre && /^(v-[A-Za-z0-9-]|:|\.|@|#)/.test(name)) {
    ...
    if (dirName === "bind" && arg) {
      ...
      if (
        (process.env.NODE_ENV !== "production") && modifiers.includes("prop")
      ) {
        checkCompatEnabled(
          "COMPILER_V_BIND_PROP", /* COMPILER_V_BIND_PROP */
          context,
...
function processIf(node, dir, context, processCodegen) {
  ...
  if (dir.name === "if") {
    ...
    while (i-- >= -1) {
      const sibling = siblings[i];
      if (
        (process.env.NODE_ENV !== "production") && sibling &&
  ...
  (node, dir, context) => {
    ...
    return processFor(node, dir, context, (forNode) => {
      ...
      return () => {
        ...
        const slotOutlet = isSlotOutlet(node) ? node : isTemplate &&
...
function buildSlots(node, context, buildSlotFn = buildClientSlotFn) {
  ...
  let slots = createObjectExpression(
    slotsProperties.concat(createObjectProperty(
      `_`,
      // 2 = compiled but dynamic = can skip normalization, but must run diff
      // 1 = compiled and static = can skip normalization AND diff as optimized
      createSimpleExpression(
        slotFlag + ((process.env.NODE_ENV !== "production")
...
const transformElement = (node, context) => {
  ...
  return function postTransformElement() {
    ...
    let vnodeTag = isComponent
      ...
      : `"${tag}"`;
    const isDynamicComponent = isObject(vnodeTag) &&
    ...
    if (node.children.length > 0) {
      if (vnodeTag === KEEP_ALIVE) {
        ...
        if (
          (process.env.NODE_ENV !== "production") && node.children.length > 1
        ) {
          context.onError(
...
var require$$0 = /*@__PURE__*/ build.getAugmentedNamespace(
...
(function (exports) {
  ...
  const resolveModifiers = (key, modifiers, context, loc) => {
    ...
    for (let i = 0; i < modifiers.length; i++) {
      ...
      if (
        modifier === "native" &&
        compilerCore.checkCompatEnabled(
          "COMPILER_V_ON_NATIVE", /* COMPILER_V_ON_NATIVE */
          context,
          loc,
        )
  ...
  const transformClick = (key, event) => {
    const isStaticClick = compilerCore.isStaticExp(key) &&
  ...
  const transformOn = (dir, node, context) => {
    return compilerCore.transformOn(dir, node, context, (baseResult) => {
      ...
      if (eventOptionModifiers.length) {
        const modifierPostfix = eventOptionModifiers.map(shared.capitalize)
          .join("");
    ...
    for (let i = 0; i < node.props.length; i++) {
      ...
      if (p.type === 6 /* ATTRIBUTE */) {
        ...
        if (exp.content[0] === "_") {
          ...
          res +=
            ` ${p.arg.content}="__VUE_EXP_START__${exp.content}__VUE_EXP_END__"`;
          continue;
        }
...
var compilerDom_cjs = /*@__PURE__*/ build.getDefaultExportFromCjs(
  compilerDom_cjs$2,
);

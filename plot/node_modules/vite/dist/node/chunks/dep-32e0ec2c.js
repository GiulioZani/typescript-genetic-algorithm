...
var freeGlobal = typeof build$2.commonjsGlobal == "object" &&
  build$2.commonjsGlobal && build$2.commonjsGlobal.Object === Object &&
    ...
    if (typeof data === "string") {
      while (data.length > MAX_SHORT_STRING$1) {
        this._updateWithShortString(
          data.slice(0, MAX_SHORT_STRING$1),
          encoding,
        );
...
function interpolateName$1(loaderContext, name, options = {}) {
  ...
  const hasQuery = loaderContext.resourceQuery &&
    loaderContext.resourceQuery.length > 1;
...
function replaceAll(replacements, text) {
  ...
  while (matches = matchConstName.exec(text)) {
    ...
    if (replacement) {
      text = text.slice(0, matches.index) + replacement +
...
class Parser {
  ...
  plugin() {
    ...
    return {
      ...
      OnceExit(css) {
        return Promise.all(parser.fetchAllImports(css)).then(() =>
...
class Core {
  ...
  load(sourceString, sourcePath, trace, pathFetcher) {
    ...
    return (0, _postcss2$1.default)(this.plugins.concat([parser.plugin()]))
      .process(sourceString, { from: "/" + sourcePath }).then((result) => {
        return {
  ...
  var Node = /*#__PURE__*/ function () {
    ...
    _proto.isAtPosition = function isAtPosition(line, column) {
      if (this.source && this.source.start && this.source.end) {
        ...
        if (
          this.source.start.line === line && this.source.start.column > column
        ) {
          return false;
        }
    ...
    _proto.toString = function toString() {
      return [this.rawSpaceBefore, this.valueToString(), this.rawSpaceAfter]
        .join("");
    };
...
types.UNIVERSAL = types.ATTRIBUTE = types.CLASS = types.COMBINATOR = types
  .COMMENT = types.ID = types.NESTING = types.PSEUDO = types.ROOT = types
    .SELECTOR = types.STRING = types.TAG = void 0;
var TAG = "tag";
types.TAG = TAG;
var STRING = "string";
types.STRING = STRING;
...
(function (exports) {
  ...
  var Attribute = /*#__PURE__*/ function (_Namespace) {
    ...
    function Attribute(opts) {
      ...
      _this = _Namespace.call(this, handleDeprecatedContructorOpts(opts)) ||
        this;
      ...
      if (numSingleQuotes + numDoubleQuotes === 0) {
        ...
        if (escaped === v) {
          ...
          if (pref === Attribute.NO_QUOTE) {
            ...
            var quote = this.quoteMark || options.quoteMark ||
        ...
        selector.push(
          this._stringFor(
            ...
            function (attrValue, attrSpaces) {
              if (
                attrValue.length > 0 && !_this2.quoted &&
...
tokenTypes.combinator = tokenTypes.word = tokenTypes.comment = tokenTypes.str =
  tokenTypes.tab = tokenTypes.newline = tokenTypes.feed = tokenTypes.cr =
    tokenTypes.backslash = tokenTypes.bang = tokenTypes.slash = tokenTypes
      .doubleQuote = tokenTypes.singleQuote = tokenTypes.space = tokenTypes
        .greaterThan = tokenTypes.pipe = tokenTypes.equals = tokenTypes.plus =
          tokenTypes.caret = tokenTypes.tilde = tokenTypes.dollar = tokenTypes
            .closeSquare = tokenTypes.openSquare = tokenTypes.closeParenthesis =
              tokenTypes.openParenthesis = tokenTypes.semicolon = tokenTypes
                .colon = tokenTypes.comma = tokenTypes.at = tokenTypes
                  .asterisk = tokenTypes.ampersand = void 0;
      ...
      switch (code) {
        ...
        case t.feed:
          ...
          do {
            ...
            code === t.space || code === t.newline || code === t.tab ||
            code === t.cr || code === t.feed
          );
        ...
        case t.pipe:
          ...
          do {
            ...
            code === t.plus || code === t.greaterThan || code === t.tilde ||
            code === t.pipe
          );
...
(function (module, exports) {
  ...
  var Parser = /*#__PURE__*/ function () {
    ...
    _proto.attribute = function attribute() {
      ...
      if (
        len === 1 && !~[tokens.word].indexOf(attr[0][_tokenize.FIELDS.TYPE])
      ) {
        ...
        switch (token[_tokenize.FIELDS.TYPE]) {
          case tokens.space:
            ...
            if (lastAdded) {
              ...
              var existingComment = (0, _util.getProp)(
                node,
                "raws",
                "spaces",
                lastAdded,
          ...
          case tokens.asterisk:
            if (next[_tokenize.FIELDS.TYPE] === tokens.equals) {
              ...
              (!node.namespace ||
                lastAdded === "namespace" && !spaceAfterMeaningfulToken) && next
            ) {
              if (spaceBefore) {
              ...
              var rawValue = (0, _util.getProp)(node, "raws", "namespace") ||
                null;
          ...
          case tokens.combinator:
            if (
              content === "~" && next[_tokenize.FIELDS.TYPE] === tokens.equals
            ) {
              node.operator = content;
              lastAdded = "operator";
            }
          ...
          case tokens.word:
            if (
              next && this.content(next) === "|" && attr[pos + 2] &&
              ...
              var _rawValue = (0, _util.getProp)(node, "raws", "attribute") ||
                null;
          ...
          case tokens.comment:
            if (lastAdded) {
              if (
                ...
                var rawLastComment = (0, _util.getProp)(
                  node,
                  "raws",
                  "spaces",
                  lastAdded,
    ...
    _proto.combinator = function combinator() {
      ...
      if (
        this.currToken && this.currToken[_tokenize.FIELDS.TYPE] === tokens.space
      ) {
    ...
    _proto.pseudo = function pseudo() {
      ...
      while (
        this.currToken && this.currToken[_tokenize.FIELDS.TYPE] === tokens.colon
      ) {
    ...
    _proto.expected = function expected(description, index, found) {
      ...
      return this.error(
        "Expected " + an + " " + description + ', found "' + found +
          '" instead.',
        {
          index: index,
        },
...
constructors.universal = constructors.tag = constructors.string = constructors
  .selector = constructors.root = constructors.pseudo = constructors.nesting =
    constructors.id = constructors.comment = constructors.combinator =
      constructors.className = constructors.attribute = void 0;
...
guards.isUniversal = guards.isTag = guards.isString = guards.isSelector = guards
  .isRoot = guards.isPseudo = guards.isNesting = guards.isIdentifier = guards
    .isComment = guards.isCombinator = guards.isClassName = guards.isAttribute =
      void 0;
...
function isPseudoElement(node) {
  return isPseudo(node) && node.value &&
    (node.value.startsWith("::") || node.value.toLowerCase() === ":before" ||
...
const replaceValueSymbols$2 = (value, replacements) => {
  ...
  while ((matches = matchValueName.exec(value))) {
    ...
    if (replacement) {
      value = value.slice(0, matches.index) +
        replacement +
  ...
  const transform = (node, context) => {
    ...
    switch (node.type) {
      ...
      case "pseudo": {
        ...
        const isImportExport = node.value === ":import" ||
          node.value === ":export";
    ...
    const valueNodes = valueParser(declaration.value).walk((node) => {
      ...
      if (value && validIdent.test(value)) {
        if ("$" + value in animationKeywords) {
          ...
          shouldParseAnimationName = parsedAnimationKeywords["$" + value] >=
          ...
          root.walkDecls(/^composes$/, (declaration) => {
            ...
            if (global) {
              ...
              while (parent.type !== "root") {
                parentIndexes = parent.parent.index(parent) + "_" +
                  parentIndexes;
...
const plugin = (options = {}) => {
  const generateScopedName = (options && options.generateScopedName) ||
    plugin.generateScopedName;
  const generateExportEntry = (options && options.generateExportEntry) ||
...
src.exports = (options) => {
  let importIndex = 0;
  const createImportedName = (options && options.createImportedName) ||
    ((importName /*, path*/) =>
...
var _postcssModulesLocalByDefault2 = _interopRequireDefault$1(
  _postcssModulesLocalByDefault,
);

var _postcssModulesExtractImports = src$2.exports;

var _postcssModulesExtractImports2 = _interopRequireDefault$1(
  _postcssModulesExtractImports,
);
...
function isValidBehaviour(behaviour) {
  return Object.keys(behaviours).map((key) => behaviours[key]).indexOf(
    behaviour,
  ) > -1;
}
...
function getDefaultScopeBehaviour(opts) {
  if (
    opts.scopeBehaviour &&
    (0, _behaviours.isValidBehaviour)(opts.scopeBehaviour)
  ) {
    ...
    OnceExit(css, { result }) {
      return _asyncToGenerator(function* () {
        ...
        const fetcher = function fetcher(file, relativeTo, depTrace) {
          ...
          const resolvedResult = typeof opts.resolve === "function" &&
            opts.resolve(unquoteFile);
          const resolvedFile = resolvedResult instanceof Promise
            ? resolvedResult
        ...
        if (opts.localsConvention) {
          ...
          parser.exportTokens = Object.entries(parser.exportTokens).reduce(
            function (tokens, [className, value]) {
              if (isFunc) {
                tokens[opts.localsConvention(className, value, inputFile)] =
                  value;

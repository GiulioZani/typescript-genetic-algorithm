...
async function watch(command) {
  ...
  const configFile = command.config
    ? await cli.getConfigPath(command.config)
    : null;
  onExit(close);
  ...
  async function loadConfigFromFileAndTrack(configFile) {
    ...
    configWatcher = index.chokidar.watch(configFile).on(
      "change",
      reloadConfigFile,
    );
    ...
    async function reloadConfigFile() {
      try {
        const newConfigFileData = await require$$0$2.promises.readFile(
          configFile,
          "utf8",
        );
        ...
        const { options, warnings } = await loadConfigFile_js
    ...
    watcher.on("event", (event) => {
      switch (event.code) {
        ...
        case "START":
          if (!silent) {
            ...
            resetScreen(
              loadConfigFile_js.underline(`rollup v${rollup.version}`),
            );
          }
          break;
        case "BUNDLE_START":
          if (!silent) {
            ...
            loadConfigFile_js.stderr(
              loadConfigFile_js.cyan(
                `bundles ${loadConfigFile_js.bold(input)} → ${
                  loadConfigFile_js.bold(
                    event.output.map(rollup.relativeId).join(", "),
                  )
                }...`,
              ),
            );
          ...
          if (!silent) {
            loadConfigFile_js.stderr(
              loadConfigFile_js.green(
                ...
                } in ${loadConfigFile_js.bold(cli.ms(event.duration))}`,
              ),
            );
          }
      ...
      if ("result" in event && event.result) {
        event.result.close().catch((error) =>
          loadConfigFile_js.handleError(error, true)
        );
      }
    });
  }

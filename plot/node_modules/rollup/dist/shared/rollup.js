...
function errAssetNotFinalisedForFileName(name) {
  return {
    ...
    message:
      `Plugin error - Unable to get file name for asset "${name}". Ensure that the source is set and that generate is called first.`,
  };
}
...
function errChunkNotGeneratedForFileName(name) {
  return {
    ...
    message:
      `Plugin error - Unable to get file name for chunk "${name}". Ensure that generate is called first.`,
  };
}
function errChunkInvalid({ fileName, code }, exception) {
  const errorProps = {
    ...
    message:
      `Chunk "${fileName}" is not valid JavaScript: ${exception.message}.`,
  };
...
function errCircularReexport(exportName, importedModule) {
  return {
    ...
    id: importedModule,
    message: `"${exportName}" cannot be exported from ${
      relativeId(importedModule)
    } as it is a reexport that references itself.`,
  };
}
function errCyclicCrossChunkReexport(
  exportName,
  exporter,
  reexporter,
  importer,
  ...
  return {
    ...
    exporter,
    importer,
    message: `Export "${exportName}" of module ${
      relativeId(exporter)
    } was reexported through module ${
      relativeId(reexporter)
...
function errAssetReferenceIdNotFoundForSetSource(assetReferenceId) {
  return {
    ...
    message:
      `Plugin error - Unable to set the source for unknown asset "${assetReferenceId}".`,
  };
}
function errAssetSourceAlreadySet(name) {
  return {
    ...
    message:
      `Unable to set the source for asset "${name}", source already set.`,
  };
}
function errNoAssetSourceSet(assetName) {
  return {
    ...
    message:
      `Plugin error creating asset "${assetName}" - no asset source set.`,
  };
}
...
function errFileReferenceIdNotFoundForFilename(assetReferenceId) {
  return {
    ...
    message:
      `Plugin error - Unable to get file name for unknown file "${assetReferenceId}".`,
  };
}
function errFileNameConflict(fileName) {
  return {
    ...
    message:
      `The emitted file "${fileName}" overwrites a previously emitted file of the same name.`,
  };
}
...
function errInvalidExportOptionValue(optionValue) {
  return {
    ...
    message:
      `"output.exports" must be "default", "named", "none", "auto", or left unspecified (defaults to "auto"), received "${optionValue}"`,
...
function errIncompatibleExportOptionValue(optionValue, keys, entryModule) {
  return {
    ...
    message:
      `"${optionValue}" was specified for "output.exports", but entry module "${
        relativeId(entryModule)
...
function errInternalIdCannotBeExternal(source, importer) {
  return {
    ...
    message: `'${source}' is imported as an external by ${
      relativeId(importer)
...
function errMissingExport(exportName, importingModule, importedModule) {
  return {
    ...
    message: `'${exportName}' is not exported by ${
      relativeId(importedModule)
...
function errImplicitDependantCannotBeExternal(
  ...
  return {
    ...
    message: `Module "${
      relativeId(unresolvedId)
    }" that should be implicitly loaded before "${
      relativeId(implicitlyLoadedBefore)
    }" cannot be external.`,
  };
}
...
function errImplicitDependantIsNotIncluded(module) {
  ...
  return {
    code: Errors.MISSING_IMPLICIT_DEPENDANT,
    message: `Module "${
      relativeId(module.id)
    }" that should be implicitly loaded before ${
      printQuotedStringList(implicitDependencies)
...
function errNamespaceConflict(name, reexportingModuleId, sources) {
  return {
    ...
    message: `Conflicting namespaces: "${
      relativeId(reexportingModuleId)
    }" re-exports "${name}" from one of the modules ${
      printQuotedStringList(sources.map((moduleId) => relativeId(moduleId)))
    } (will be ignored)`,
    name,
...
function errAmbiguousExternalNamespaces(
  name,
  reexportingModule,
  usedModule,
  sources,
  ...
  return {
    ...
    message: `Ambiguous external namespace resolution: "${
      relativeId(reexportingModule)
    }" re-exports "${name}" from one of the external modules ${
      printQuotedStringList(sources.map((module) => relativeId(module)))
...
function errPreferNamedExports(facadeModuleId) {
  ...
  return {
    ...
    id: facadeModuleId,
    message:
      `Entry module "${file}" is implicitly using "default" export mode, which means for CommonJS output that its default export is assigned to "module.exports". For many tools, such CommonJS output will not be interchangeable with the original ES module. If this is intended, explicitly set "output.exports" to either "auto" or "default", otherwise you might want to consider changing the signature of "${file}" to use named exports only.`,
    url: `https://rollupjs.org/guide/en/#outputexports`,
  };
}
function errSyntheticNamedExportsNeedNamespaceExport(
  id,
  ...
  return {
    ...
    message: `Module "${
      ...
      JSON.stringify(syntheticNamedExportsOption)
    }' needs ${
      typeof syntheticNamedExportsOption === "string" &&
        syntheticNamedExportsOption !== "default"
...
function errUnexpectedNamedImport(id, imported, isReexport) {
  ...
  return {
    ...
    id,
    message:
      `The named export "${imported}" was ${importType}ed from the external module ${
        relativeId(id)
...
function errExternalSyntheticExports(source, importer) {
  return {
    ...
    importer: relativeId(importer),
    message:
      `External '${source}' can not have 'syntheticNamedExports' enabled.`,
    source,
  };
}
...
function warnUnknownOptions(
  ...
  const unknownOptions = Object.keys(passedOptions).filter((key) =>
    !(validOptionSet.has(key) || ignoredKeys.test(key))
  );
...
const objectifyOption = (value) =>
  value && typeof value === "object" ? value : {};
const objectifyOptionWithPresets = (presets, optionName, additionalValues) =>
  (value) => {
    if (typeof value === "string") {
      ...
      error(
        errInvalidOption(
          ...
          `valid values are ${additionalValues}${
            printQuotedStringList(Object.keys(presets))
...
const getOptionWithPreset = (value, presets, optionName, additionalValues) => {
  var _a;
  const presetName = (_a = value) === null || _a === void 0
    ? void 0
    : _a.preset;
  if (presetName) {
  ...
  return objectifyOptionWithPresets(presets, optionName, additionalValues)(
    value,
  );
};
const getHashFromObjectOption = (optionName) =>
...
async function loadFsEvents() {
  try {
    ({ default: fsEvents } = await Promise.resolve().then(() =>
      /*#__PURE__*/ _interopNamespaceDefault(require("fsevents"))
    ));
  } catch (err) {
...
MagicString.prototype.clone = function clone() {
  ...
  var clonedChunk =
    (cloned.firstChunk = cloned.lastSearchedChunk = originalChunk.clone());
...
MagicString.prototype._splitChunk = function _splitChunk(chunk, index) {
  if (chunk.edited && chunk.content.length) {
    ...
    throw new Error(
      "Cannot split a chunk that has already been edited (" + (loc.line) + ":" +
      (loc.column) + ' – "' + (chunk.original) + '")',
    );
  }
...
Bundle$1.prototype.addSource = function addSource(source) {
  ...
  if (source.filename) {
    if (!hasOwnProp.call(this.uniqueSourceIndexByFilename, source.filename)) {
      ...
      var uniqueSource =
        this.uniqueSources[this.uniqueSourceIndexByFilename[source.filename]];
      if (source.content.original !== uniqueSource.content) {
        throw new Error(
          "Illegal source: same filename (" + (source.filename) +
          "), different contents",
        );
      }
    }
...
Bundle$1.prototype.generateDecodedMap = function generateDecodedMap(options) {
  ...
  this.sources.forEach(function (source, i) {
    ...
    var sourceIndex = source.filename
      ? this$1$1.uniqueSourceIndexByFilename[source.filename]
      : -1;
...
class PathTracker {
  ...
  getEntities(path) {
    ...
    for (const pathSegment of path) {
      currentPaths = currentPaths[pathSegment] = currentPaths[pathSegment] ||
...
function makeLegal(str) {
  str = str.replace(/-(\w)/g, (_, letter) => letter.toUpperCase()).replace(
    illegalCharacters,
    "_",
  );
  if (startsWithDigit(str) || RESERVED_NAMES$1.has(str)) {
...
class ExternalModule {
  ...
  setRenderPath(options, inputBase) {
    this.renderPath = typeof options.paths === "function"
  ...
  warnUnusedImports() {
    const unused = Object.keys(this.declarations).filter((name) => {
      ...
      return !declaration.included && !this.reexported &&
    ...
    this.options.onwarn({
      ...
      message: `${
        printQuotedStringList(unused, ["is", "are"])
      } imported from external module "${this.id}" but never used in ${
        printQuotedStringList(importersArray.map((importer) =>
          relativeId(importer)
        ))
      }.`,
      names: unused,
  ...
  const extglobClose = (token) => {
    ...
    if (token.type === "negate") {
      ...
      if (
        ...
        const expression =
          parse$1(rest, { ...options, fastpaths: false }).output;
...
picomatch$1.matchBase = (
  ...
  const regex = glob instanceof RegExp
    ? glob
    : picomatch$1.makeRe(glob, options);
...
picomatch$1.isMatch = (str, patterns, options) =>
...
function getMatcherString(id, resolutionBase) {
  if (
    resolutionBase === false || require$$0.isAbsolute(id) || id.startsWith("*")
  ) {
    return id;
  }
...
const forbiddenIdentifiers = new Set(
  `${reservedWords$1} ${builtins$1}`.split(" "),
);
...
base$1.Program = base$1.BlockStatement = base$1.StaticBlock = function (
  node,
  st,
  c,
) {
...
base$1.ExpressionStatement = base$1.ParenthesizedExpression = base$1
...
base$1.ReturnStatement = base$1.YieldExpression = base$1.AwaitExpression =
  function (node, st, c) {
...
base$1.ImportSpecifier = base$1.ImportDefaultSpecifier = base$1
  .ImportNamespaceSpecifier = base$1.Identifier = base$1.PrivateIdentifier =
    base$1.Literal = ignore;
...
base$1.MethodDefinition = base$1.PropertyDefinition = base$1.Property =
  function (node, st, c) {
...
const whiteSpaceNoNewline =
  "[ \\f\\r\\t\\v\\u00a0\\u1680\\u2000-\\u200a\\u2028\\u2029\\u202f\\u205f\\u3000\\ufeff]";
const SOURCEMAPPING_URL_RE = new RegExp(
  `^#${whiteSpaceNoNewline}+${exports.SOURCEMAPPING_URL}=.+`,
);
...
function handlePureAnnotationsOfNode(node, state, type = node.type) {
  ...
  if (comment && comment.end <= node.end) {
    ...
    while (
      (comment = annotations[state.annotationIndex]) && comment.end <= node.end
    ) {
    ...
    while (true) {
      ...
      switch (node.type) {
        ...
        case SequenceExpression$1:
          ...
          if (
            doesNotMatchOutsideComment(
              code.slice(parentStart, node.start),
              noWhitespace,
            )
          ) {
  ...
  while ((nextMatch = forbiddenChars.exec(code)) !== null) {
    if (nextMatch[0] === "/") {
      ...
      if (charCodeAfterSlash === 42 /*"*"*/) {
        forbiddenChars.lastIndex =
          code.indexOf("*/", forbiddenChars.lastIndex + 1) + 2;
        continue;
...
function getAndCreateKeys(esTreeNode) {
  keys[esTreeNode.type] = Object.keys(esTreeNode).filter((key) =>
    typeof esTreeNode[key] === "object" && key.charCodeAt(0) !== 95 /* _ */
  );
...
class Method extends ExpressionEntity {
  ...
  deoptimizeThisOnEventAtPath(event, path, thisParameter) {
    if (
      event === EVENT_CALLED && path.length === 0 &&
  ...
  hasEffectsWhenCalledAtPath(path, callOptions, context) {
    ...
    if (
      ...
      (this.description.mutatesSelfAsArray === true &&
        ((_a = callOptions.thisParam) === null || _a === void 0
          ? void 0
          : _a.hasEffectsWhenAssignedAtPath(UNKNOWN_INTEGER_PATH, context)))
    ) {
      return true;
    }
    ...
    for (const argIndex of this.description.callsArgs) {
      if (
        (_b = callOptions.args[argIndex]) === null || _b === void 0
          ? void 0
          : _b.hasEffectsWhenCalledAtPath(EMPTY_PATH, {
            args: NO_ARGS,
            thisParam: null,
            withNew: false,
          }, context)
...
class ObjectEntity extends ExpressionEntity {
  ...
  constructor(properties, prototypeExpression, immutable = false) {
    ...
    if (Array.isArray(properties)) {
      ...
      this.propertiesAndGettersByKey = this.propertiesAndSettersByKey =
        properties;
  ...
  deoptimizeAllProperties() {
    ...
    for (
      const properties of Object.values(this.propertiesAndGettersByKey).concat(
    ...
    (_a = this.prototypeExpression) === null || _a === void 0
      ? void 0
      : _a.deoptimizePath([UnknownKey, UnknownKey]);
  ...
  deoptimizeIntegerProperties() {
    ...
    for (
      const [key, propertiesAndGetters] of Object.entries(
  ...
  deoptimizePath(path) {
    ...
    for (
      const property of typeof key === "string"
        ? (this.propertiesAndGettersByKey[key] ||
    ...
    (_a = this.prototypeExpression) === null || _a === void 0
      ? void 0
      : _a.deoptimizePath(path.length === 1 ? [UnknownKey, UnknownKey] : path);
  }
  deoptimizeThisOnEventAtPath(event, path, thisParameter, recursionTracker) {
    var _a;
    ...
    const [
      propertiesForExactMatchByKey,
      relevantPropertiesByKey,
    ...
    if (typeof key === "string") {
      ...
      for (
        const properties of Object.values(relevantPropertiesByKey).concat([
          relevantUnmatchableProperties,
        ])
      ) {
        for (const property of properties) {
    ...
    (_a = this.prototypeExpression) === null || _a === void 0
      ? void 0
      : _a.deoptimizeThisOnEventAtPath(
        event,
        path,
        thisParameter,
        recursionTracker,
  ...
  hasEffectsWhenAccessedAtPath(path, context) {
    ...
    if (path.length > 1) {
      ...
      if (this.prototypeExpression) {
        return this.prototypeExpression.hasEffectsWhenAccessedAtPath(
          path,
          context,
        );
      }
    ...
    if (typeof key === "string") {
      ...
      for (
        const getters of Object.values(this.gettersByKey).concat([
          this.unmatchableGetters,
        ])
      ) {
        for (const getter of getters) {
  ...
  hasEffectsWhenCalledAtPath(path, callOptions, context) {
    ...
    const expressionAtPath = this.getMemberExpression(key);
    if (expressionAtPath) {
      return expressionAtPath.hasEffectsWhenCalledAtPath(
        path.slice(1),
        callOptions,
        context,
      );
    ...
    if (this.prototypeExpression) {
      return this.prototypeExpression.hasEffectsWhenCalledAtPath(
        path,
        callOptions,
        context,
      );
  ...
  deoptimizeCachedEntities() {
    for (
      const expressionsToBeDeoptimized of Object.values(
  ...
  deoptimizeCachedIntegerEntities() {
    for (
      const [key, expressionsToBeDeoptimized] of Object.entries(
        this.expressionsToBeDeoptimizedByKey,
      )
    ) {
      if (INTEGER_REG_EXP.test(key)) {
...
class ArrayExpression extends NodeBase {
  ...
  hasEffectsWhenCalledAtPath(path, callOptions, context) {
    return this.getObjectEntity().hasEffectsWhenCalledAtPath(
      path,
      callOptions,
      context,
    );
  ...
  deoptimizePath(path) {
    ...
    if (path.length === 0) {
      if (!this.isReassigned) {
        ...
        (_a = this.init) === null || _a === void 0
          ? void 0
          : _a.deoptimizePath(UNKNOWN_PATH);
      }
    } else {
      (_b = this.init) === null || _b === void 0
        ? void 0
        : _b.deoptimizePath(path);
    }
  }
  deoptimizeThisOnEventAtPath(event, path, thisParameter, recursionTracker) {
...
class ChildScope extends Scope$1 {
  ...
  findVariable(name) {
    const knownVariable = this.variables.get(name) ||
      this.accessedOutsideVariables.get(name);
    if (knownVariable) {
...
function renderStatementList(statements, code, start, end, options) {
  ...
  for (let nextIndex = 1; nextIndex <= statements.length; nextIndex++) {
    ...
    nextNodeNeedsBoundaries = nextNode === undefined
      ? false
      : !nextNode.included || nextNode.needsBoundaries;
    if (currentNodeNeedsBoundaries || nextNodeNeedsBoundaries) {
      nextNodeStart = currentNode.end +
        findFirstLineBreakOutsideComment(
          code.original.slice(
            currentNode.end,
...
function getCommaSeparatedNodesWithBoundaries(nodes, code, start, end) {
  ...
  for (let nextIndex = 0; nextIndex < nodes.length; nextIndex++) {
    ...
    nextNodeStart = contentEnd = separator +
      1 +
      findFirstLineBreakOutsideComment(
...
class BlockScope extends ChildScope {
  addDeclaration(identifier, context, init, isHoisted) {
    if (isHoisted) {
      const variable = this.parent.addDeclaration(
        identifier,
        context,
        init,
        isHoisted,
...
class BlockStatement extends NodeBase {
  ...
  initialise() {
    ...
    this.deoptimizeBody = firstBodyStatement instanceof ExpressionStatement &&
...
class Identifier extends NodeBase {
  ...
  hasEffectsWhenAccessedAtPath(path, context) {
    return (this.variable !== null &&
      this.getVariableRespectingTDZ().hasEffectsWhenAccessedAtPath(
        path,
        context,
      ));
  }
  ...
  hasEffectsWhenCalledAtPath(path, callOptions, context) {
    return (!this.variable ||
      this.getVariableRespectingTDZ().hasEffectsWhenCalledAtPath(
        path,
        callOptions,
        context,
      ));
  ...
  render(
    ...
    { renderedParentType, isCalleeOfRenderedParent, isShorthandProperty } =
      BLANK,
  ) {
    if (this.variable) {
...
class ArrowFunctionExpression extends NodeBase {
  ...
  hasEffectsWhenCalledAtPath(path, _callOptions, context) {
    ...
    if (this.async) {
      ...
      if (
        returnExpression.hasEffectsWhenCalledAtPath(["then"], {
          args: NO_ARGS,
          thisParam: null,
          withNew: false,
        }, context) ||
...
function getSystemExportStatement(
  ...
  if (
    ...
    return `exports('${exportNamesByVariable.get(variable)}',${_}${
...
function renderSystemExportFunction(
  exportedVariables,
  expressionStart,
  expressionEnd,
  needsParens,
...
function renderSystemExportSequenceAfterExpression(
  ...
  code.appendLeft(
    expressionEnd,
    `,${_}${getSystemExportStatement([exportedVariable], options)},${_}${
      exportedVariable.getName(getPropertyAccess)
    }`,
  );
  if (needsParens) {
...
class AssignmentExpression extends NodeBase {
  ...
  render(
    code,
    options,
    { preventASI, renderedParentType, renderedSurroundingElement } = BLANK,
  ) {
    if (this.left.included) {
      ...
      const inclusionStart = findNonWhiteSpace(
        code.original,
        findFirstOccurrenceOutsideComment(code.original, "=", this.left.end) +
          1,
      );
        ...
        if (exportNames) {
          if (exportNames.length === 1) {
            ...
            renderSystemExportSequenceAfterExpression(
              ...
              this.start,
              this.end,
              this.parent.type !== ExpressionStatement$1,
              code,
              options,
            );
          }
...
class MemberExpression extends NodeBase {
  ...
  getPropertyKey() {
    if (this.propertyKey === null) {
      ...
      return (this.propertyKey = value === UnknownValue
        ? UnknownKey
        : String(value));
    }
...
class CallExpression extends NodeBase {
  ...
  hasEffectsWhenCalledAtPath(path, callOptions, context) {
    return (!(callOptions.withNew ? context.instantiated : context.called)
      .trackEntityAtPathAndGetIfTracked(path, callOptions, this) &&
      this.getReturnExpression().hasEffectsWhenCalledAtPath(
        path,
        callOptions,
        context,
      ));
  ...
  render(code, options, { renderedSurroundingElement } = BLANK) {
    ...
    if (this.arguments.length > 0) {
      if (this.arguments[this.arguments.length - 1].included) {
        ...
        while (
          lastIncludedIndex >= 0 && !this.arguments[lastIncludedIndex].included
        ) {
          lastIncludedIndex--;
        }
  ...
  getReturnExpression(recursionTracker = SHARED_RECURSION_TRACKER) {
    if (this.returnExpression === null) {
      this.returnExpression = UNKNOWN_EXPRESSION;
      return (this.returnExpression = this.callee
        .getReturnExpressionWhenCalledAtPath(
          EMPTY_PATH,
          this.callOptions,
          recursionTracker,
...
class MethodBase extends NodeBase {
  ...
  getLiteralValueAtPath(path, recursionTracker, origin) {
    return this.getAccessedValue().getLiteralValueAtPath(
      path,
      recursionTracker,
      origin,
    );
  ...
  hasEffectsWhenCalledAtPath(path, callOptions, context) {
    return this.getAccessedValue().hasEffectsWhenCalledAtPath(
      path,
      callOptions,
      context,
    );
...
class ClassNode extends NodeBase {
  ...
  hasEffects(context) {
    var _a, _b;
    const initEffect =
      ((_a = this.superClass) === null || _a === void 0
        ? void 0
        : _a.hasEffects(context)) || this.body.hasEffects(context);
    (_b = this.id) === null || _b === void 0
      ? void 0
      : _b.markDeclarationReached();
  ...
  include(context, includeChildrenRecursively) {
    ...
    (_a = this.superClass) === null || _a === void 0
      ? void 0
      : _a.include(context, includeChildrenRecursively);
    this.body.include(context, includeChildrenRecursively);
    if (this.id) {
  ...
  getObjectEntity() {
    ...
    for (const definition of this.body.body) {
      ...
      const kind = definitionKind === "set" || definitionKind === "get"
        ? definitionKind
        : "init";
      let key;
      if (definition.computed) {
        ...
        key = definition.key instanceof Identifier
          ? definition.key.name
    ...
    staticProperties.unshift({
      ...
      property: new ObjectEntity(
        dynamicMethods,
        this.superClass
          ? new ObjectMember(this.superClass, "prototype")
          : OBJECT_PROTOTYPE,
      ),
    });
...
class ClassDeclaration extends ClassNode {
  ...
  render(code, options) {
    ...
    if (
      format === "system" && this.id &&
      exportNamesByVariable.has(this.id.variable)
    ) {
      code.appendLeft(
        this.end,
        `${_}${getSystemExportStatement([this.id.variable], options)};`,
      );
    }
    super.render(code, options);
  }
...
class ConditionalExpression extends NodeBase {
  ...
  deoptimizeCache() {
    if (this.usedBranch !== null) {
      const unusedBranch = this.usedBranch === this.consequent
        ? this.alternate
        : this.consequent;
      this.usedBranch = null;
  ...
  deoptimizeThisOnEventAtPath(event, path, thisParameter, recursionTracker) {
    this.consequent.deoptimizeThisOnEventAtPath(
      event,
      path,
      thisParameter,
      recursionTracker,
    );
    this.alternate.deoptimizeThisOnEventAtPath(
      event,
      path,
      thisParameter,
      recursionTracker,
  ...
  hasEffects(context) {
    ...
    const usedBranch = this.getUsedBranch();
    if (usedBranch === null) {
      return this.consequent.hasEffects(context) ||
  ...
  hasEffectsWhenCalledAtPath(path, callOptions, context) {
    const usedBranch = this.getUsedBranch();
    if (usedBranch === null) {
      return (this.consequent.hasEffectsWhenCalledAtPath(
        path,
        callOptions,
        context,
      ) ||
  ...
  include(context, includeChildrenRecursively) {
    ...
    const usedBranch = this.getUsedBranch();
    if (
      includeChildrenRecursively || this.test.shouldBeIncluded(context) ||
      usedBranch === null
    ) {
...
class ExportDefaultDeclaration extends NodeBase {
  ...
  initialise() {
    ...
    this.declarationName = (declaration.id && declaration.id.name) ||
  ...
  renderNamedDeclaration(
    ...
    const { exportNamesByVariable, format, snippets: { getPropertyAccess } } =
      options;
    const name = this.variable.getName(getPropertyAccess);
  ...
  renderVariableDeclaration(
    ...
    const systemExportNames = format === "system" &&
      exportNamesByVariable.get(this.variable);
    if (systemExportNames) {
      code.overwrite(
        this.start,
        declarationStart,
        `${cnst} ${this.variable.getName(getPropertyAccess)} = exports('${
          systemExportNames[0]
        }', `,
      );
      code.appendRight(
...
class IfStatement extends NodeBase {
  ...
  parseNode(esTreeNode) {
    ...
    this.consequent =
      new (this.context.getNodeConstructor(esTreeNode.consequent.type))(
    ...
    if (esTreeNode.alternate) {
      ...
      this.alternate =
        new (this.context.getNodeConstructor(esTreeNode.alternate.type))(
  ...
  render(code, options) {
    ...
    if (
      this.consequent.included &&
      (noTreeshake || testValue === UnknownValue || testValue)
    ) {
      this.consequent.render(code, options);
    } else {
      code.overwrite(
  ...
  renderHoistedDeclarations(hoistedDeclarations, code, getPropertyAccess) {
    ...
    if (hoistedVars) {
      ...
      const needsBraces = parentType !== Program$1 &&
        parentType !== BlockStatement$1;
      code.prependRight(
        this.start,
        `${needsBraces ? "{ " : ""}var ${hoistedVars}; `,
      );
      if (needsBraces) {
        code.appendLeft(this.end, ` }`);
      }
...
const isDefaultAProperty = (interopType, externalLiveBindings) =>
...
const canDefaultBeTakenFromNamespace = (interopType, externalLiveBindings) =>
...
const HELPER_GENERATORS = {
  [INTEROP_DEFAULT_LEGACY_VARIABLE](_t, snippets, liveBindings) {
    ...
    return (`${left}e${_}&&${_}typeof e${_}===${_}'object'${_}&&${_}'default'${_}in e${_}?${_}` +
      `${
        liveBindings
          ? getDefaultLiveBinding(snippets)
          : getDefaultStatic(snippets)
      }${right}${n}${n}`);
  },
...
const getDefaultLiveBinding = ({ _, getObject }) =>
  `e${_}:${_}${getObject([["default", "e"]], { lineBreakIndent: null })}`;
const getDefaultStatic = ({ _, getPropertyAccess }) =>
...
const loopOverKeys = (
  body,
  allowVarLoopVariable,
  { _, cnst, getFunctionIntro, s },
) =>
...
const loopOverNamespaces = (
  ...
  useForEach,
  t,
  { _, cnst, getDirectReturnFunction, getFunctionIntro, n },
) => {
  if (useForEach) {
    const [left, right] = getDirectReturnFunction(["e"], {
    ...
    return (`m.forEach(${left}` +
      `e${_}&&${_}typeof e${_}!==${_}'string'${_}&&${_}!Array.isArray(e)${_}&&${_}Object.keys(e).forEach(${
        getFunctionIntro(["k"], {
          isAsync: false,
          name: null,
        })
...
const copyOwnPropertyLiveBinding = (
  t,
  i,
  { _, cnst, getDirectReturnFunction, n },
) => {
...
const getFrozen = (fragment, freeze) =>
...
class ImportExpression extends NodeBase {
  ...
  render(code, options) {
    if (this.inlineNamespace) {
      const { snippets: { getDirectReturnFunction, getPropertyAccess } } =
        options;
      const [left, right] = getDirectReturnFunction([], {
        functionReturn: true,
        lineBreakIndent: null,
      ...
      code.overwrite(
        ...
        `Promise.resolve().then(${left}${
          this.inlineNamespace.getName(getPropertyAccess)
        }${right})`,
        { contentOnly: true },
      );
      return;
    ...
    if (this.mechanism) {
      code.overwrite(
        this.start,
        findFirstOccurrenceOutsideComment(code.original, "(", this.start + 6) +
          1,
        this.mechanism.left,
        { contentOnly: true },
      );
  ...
  renderFinalResolution(
    ...
    if (namespaceExportName) {
      ...
      code.prependLeft(
        this.end,
        `.then(${left}n.${namespaceExportName}${right})`,
      );
    }
  }
  setExternalResolution(
    ...
    let helper;
    ({ helper, mechanism: this.mechanism } = this
  ...
  getDynamicImportMechanismAndHelper(
    ...
    const mechanism = pluginDriver.hookFirstSync("renderDynamicImport", [
      {
        customResolution: typeof this.resolution === "string"
          ? this.resolution
          : null,
        format,
        moduleId: this.context.module.id,
        targetModuleId: this.resolution && typeof this.resolution !== "string"
          ? this.resolution.id
          : null,
      },
    ]);
    ...
    const hasDynamicTarget = !this.resolution ||
...
class LabeledStatement extends NodeBase {
  ...
  include(context, includeChildrenRecursively) {
    ...
    this.body.include(context, includeChildrenRecursively);
    if (
      includeChildrenRecursively || context.includedLabels.has(this.label.name)
    ) {
      this.label.include();
      context.includedLabels.delete(this.label.name);
      context.brokenFlow = brokenFlow;
...
class LogicalExpression extends NodeBase {
  ...
  deoptimizeCache() {
    if (this.usedBranch !== null) {
      const unusedBranch = this.usedBranch === this.left
        ? this.right
        : this.left;
      this.usedBranch = null;
      unusedBranch.deoptimizePath(UNKNOWN_PATH);
  ...
  deoptimizeThisOnEventAtPath(event, path, thisParameter, recursionTracker) {
    this.left.deoptimizeThisOnEventAtPath(
      event,
      path,
      thisParameter,
      recursionTracker,
    );
    this.right.deoptimizeThisOnEventAtPath(
      event,
      path,
      thisParameter,
      recursionTracker,
  ...
  hasEffectsWhenCalledAtPath(path, callOptions, context) {
    const usedBranch = this.getUsedBranch();
    if (usedBranch === null) {
      return (this.left.hasEffectsWhenCalledAtPath(
        path,
        callOptions,
        context,
      ) ||
  ...
  getUsedBranch() {
    if (!this.isBranchResolutionAnalysed) {
      ...
      if (leftValue === UnknownValue) {
        ...
        this.usedBranch = (this.operator === "||" && leftValue) ||
  ...
  getReferencedFileName(outputPluginDriver) {
    ...
    if (metaProperty && metaProperty.startsWith(FILE_PREFIX)) {
      return outputPluginDriver.getFileName(
        metaProperty.substr(FILE_PREFIX.length),
      );
    }
    return null;
  }
  ...
  renderFinalMechanism(code, chunkId, format, snippets, outputPluginDriver) {
    ...
    if (
      ...
      const relativePath = normalize(
        require$$0.relative(require$$0.dirname(chunkId), fileName),
      );
      let replacement;
    ...
    ]) || ((_a = importMetaMechanisms[format]) === null || _a === void 0
      ? void 0
      : _a.call(importMetaMechanisms, metaProperty, { chunkId, snippets }));
...
const getGenericImportMetaMechanism = (getUrl) =>
  (prop, { chunkId }) => {
...
const importMetaMechanisms = {
  ...
  cjs: getGenericImportMetaMechanism((chunkId) =>
    `(typeof document === 'undefined' ? ${
      getResolveUrl(`'file:' + __filename`, `(require('u' + 'rl').URL)`)
  ...
  umd: getGenericImportMetaMechanism((chunkId) =>
    `(typeof document === 'undefined' && typeof location === 'undefined' ? ${
      getResolveUrl(`'file:' + __filename`, `(require('u' + 'rl').URL)`)
  ...
  getObjectEntity() {
    ...
    for (const property of this.properties) {
      ...
      if (property.computed) {
        ...
        key = property.key instanceof Identifier
          ? property.key.name
...
class Property extends MethodBase {
  ...
  hasEffects(context) {
    ...
    const propertyReadSideEffects = this.context.options.treeshake
      .propertyReadSideEffects;
    return ((this.parent.type === "ObjectPattern" &&
      propertyReadSideEffects === "always") ||
...
class PropertyDefinition extends NodeBase {
  ...
  hasEffectsWhenAccessedAtPath(path, context) {
    return !this.value ||
      this.value.hasEffectsWhenAccessedAtPath(path, context);
  }
  hasEffectsWhenAssignedAtPath(path, context) {
    return !this.value ||
      this.value.hasEffectsWhenAssignedAtPath(path, context);
  }
  hasEffectsWhenCalledAtPath(path, callOptions, context) {
    return !this.value ||
      this.value.hasEffectsWhenCalledAtPath(path, callOptions, context);
  }
}
...
class SequenceExpression extends NodeBase {
  ...
  render(
    ...
    for (
      const { node, separator, start, end }
      ...
      if (includedNodes === 1) {
        ...
        node.render(code, options, {
          isCalleeOfRenderedParent: isCalleeOfRenderedParent &&
            node === lastNode,
...
class SwitchStatement extends NodeBase {
  ...
  hasEffects(context) {
    ...
    for (const switchCase of this.cases) {
      ...
      minBrokenFlow = context.brokenFlow < minBrokenFlow
        ? context.brokenFlow
        : minBrokenFlow;
      context.brokenFlow = brokenFlow;
    }
  ...
  include(context, includeChildrenRecursively) {
    ...
    for (let caseIndex = this.cases.length - 1; caseIndex >= 0; caseIndex--) {
      ...
      if (isCaseIncluded) {
        ...
        minBrokenFlow = minBrokenFlow < context.brokenFlow
          ? minBrokenFlow
          : context.brokenFlow;
        context.brokenFlow = brokenFlow;
      } else {
...
class ModuleScope extends ChildScope {
  ...
  findVariable(name) {
    ...
    const variable = this.context.traceVariable(name) ||
...
class ThisExpression extends NodeBase {
  ...
  initialise() {
    this.alias = this.scope.findLexicalBoundary() instanceof ModuleScope
...
function isReassignedExportsMember(variable, exportNamesByVariable) {
  return (variable.renderBaseName !== null &&
    exportNamesByVariable.has(variable) && variable.isReassigned);
}
  ...
  if (options.format === "system") {
    for (const { node } of separatedNodes) {
      if (
        ...
        ((_a = options.exportNamesByVariable.get(node.id.variable)) === null ||
              _a === void 0
            ? void 0
            : _a.length) === 1
      ) {
...
class NamespaceVariable extends Variable {
  ...
  getMemberVariables() {
    ...
    for (
      const name of this.context.getExports().concat(
      ...
      for (const segment of line) {
        if (segment[0] >= location.column) {
          ...
          location = {
            column: segment[3],
            line: segment[2] + 1,
            name: segment.length === 5
              ? sourcemap.names[segment[4]]
              : undefined,
  ...
  if (searchedModules) {
    if (searchedModules.has(target)) {
      return isExportAllSearch
        ? [null]
        : error(errCircularReexport(name, target.id));
    }
...
function getAndExtendSideEffectModules(variable, module) {
  ...
  while (true) {
    ...
    currentVariable = currentVariable instanceof ExportDefaultVariable
    ...
    const originalSideEffects = importingModule.sideEffectDependenciesByVariable
      .get(currentVariable);
...
class Module {
    ...
    this.info = {
      ...
      get dynamicallyImportedIdResolutions() {
        return dynamicImports
          .map(({ argument }) =>
            typeof argument === "string" && module.resolvedIds[argument]
          )
          .filter(Boolean);
      },
      ...
      get hasDefaultExport() {
        ...
        return module.exports.has("default") ||
          reexportDescriptions.has("default");
      },
      get hasModuleSideEffects() {
        warnDeprecation(
          "Accessing ModuleInfo.hasModuleSideEffects from plugins is deprecated. Please use ModuleInfo.moduleSideEffects instead.",
      ...
      get importedIdResolutions() {
        return Array.from(sources, (source) => module.resolvedIds[source])
          .filter(Boolean);
      },
      get importedIds() {
        return Array.from(sources, (source) => {
          var _a;
          return (_a = module.resolvedIds[source]) === null || _a === void 0
            ? void 0
            : _a.id;
        }).filter(Boolean);
      },
  ...
  getDependenciesToBeIncluded() {
    ...
    for (let variable of dependencyVariables) {
      const sideEffectDependencies = this.sideEffectDependenciesByVariable.get(
        variable,
      );
    ...
    if (
      !this.options.treeshake || this.info.moduleSideEffects === "no-treeshake"
    ) {
  ...
  addDynamicImport(node) {
    ...
    if (argument instanceof TemplateLiteral) {
      ...
      argument instanceof Literal && typeof argument.value === "string"
    ) {
    ...
    if (location) {
      ...
      try {
        ...
        this.options.onwarn({
          ...
          message:
            `Error when using sourcemap for reporting an error: ${err.message}`,
          pos,
        });
      }
  ...
  addRelevantSideEffectDependencies(
    ...
    const addSideEffectDependencies = (possibleDependencies) => {
      for (const dependency of possibleDependencies) {
        ...
        if (
          !(dependency.info.moduleSideEffects ||
            alwaysCheckedDependencies.has(dependency))
        ) {
          continue;
        }
  ...
  getVariableFromNamespaceReexports(
    name,
    importerForSideEffects,
    searchedNamesAndModules,
  ) {
  ...
  includeDynamicImport(node) {
    const resolution =
      this.dynamicImports.find((dynamicImport) => dynamicImport.node === node)
        .resolution;
    ...
    if (!variable.included) {
      ...
      if (variableModule instanceof Module) {
        ...
        if (variableModule !== this) {
          const sideEffectModules = getAndExtendSideEffectModules(
            variable,
            this,
          );
  ...
  shimMissingExport(name) {
    this.options.onwarn({
      ...
      exportName: name,
      message: `Missing export "${name}" has been shimmed in module ${
        relativeId(this.id)
      }.`,
    });
...
function getExportBlock$1(
  ...
  if (!namedExportsMode) {
    return `${n}${n}${mechanism}${
      getSingleDefaultExport(
        exports,
        dependencies,
        interop,
        externalLiveBindings,
      ...
      for (const specifier of reexports) {
        if (specifier.reexported !== "*") {
          ...
          if (specifier.imported !== "*" && specifier.needsLiveBinding) {
            ...
            exportBlock += `exports${
              getPropertyAccess(specifier.reexported)
            }${_}=${_}${importName};`;
          }
        }
      }
    ...
    if (reexports && namedExportsMode) {
      for (const specifier of reexports) {
        if (specifier.reexported === "*") {
          ...
          const copyPropertyIfNecessary =
            `{${n}${t}if${_}(k${_}!==${_}'default'${_}&&${_}!exports.hasOwnProperty(k))${_}${
              getDefineProperty(name, specifier.needsLiveBinding, t, snippets)
            }${s}${n}}`;
          exportBlock += cnst === "var" && specifier.needsLiveBinding
...
function getNamespaceMarkers(
  hasNamedExports,
  addEsModule,
  addNamespaceToStringTag,
  _,
  n,
) {
...
function warnOnBuiltins(warn, dependencies) {
  const externalBuiltins = dependencies.map(({ id }) => id).filter((id) =>
    id in builtins
  );
  ...
  warn({
    ...
    message:
      `Creating a browser bundle that depends on Node.js built-in modules (${
        printQuotedStringList(externalBuiltins)
...
function amd(
  ...
  const deps = dependencies.map((m) =>
    `'${removeExtensionFromRelativeAmdId(m.id)}'`
  );
  ...
  return (magicString
    ...
    .prepend(`${amd.define}(${params}(${
      getNonArrowFunctionIntro(args, {
        isAsync: false,
        name: null,
      })
    }{${useStrict}${n}${n}`)
...
function getImportBlock$1(dependencies, { _, cnst, n }, compact) {
  ...
  for (const { id, name, reexports, imports } of dependencies) {
    if (!reexports && !imports) {
      ...
      importBlock += compact && definingVariable
        ? ","
        : `${importBlock ? `;${n}` : ""}${cnst} `;
...
function es(
  ...
  { externalLiveBindings, freeze, namespaceToStringTag },
) {
  const { _, n } = snippets;
  ...
  for (const { id, reexports, imports, name } of dependencies) {
    ...
    if (imports) {
      ...
      if (defaultImport && importedNames.length === 0) {
        ...
        importBlock.push(
          `import ${defaultImport ? `${defaultImport.local},${_}` : ""}{${_}${
            importedNames
              .map((specifier) => {
...
function getExportBlock(exports, { _, cnst }) {
  ...
  for (const specifier of exports) {
    if (specifier.expression) {
      exportBlock.push(
        `${cnst} ${specifier.local}${_}=${_}${specifier.expression};`,
      );
    }
...
const keypath = (keypath, getPropertyAccess) =>
...
function iife(
  ...
  if (name && useVariableAssignment && !isLegal(name)) {
    return error({
      ...
      message:
        `Given name "${name}" is not a legal JS identifier. If you need this, you can try "output.extend: true".`,
    });
  }
  warnOnBuiltins(warn, dependencies);
  ...
  if (namedExportsMode && hasExports) {
    if (extend) {
      deps.unshift(
        `this${keypath(name, getPropertyAccess)}${_}=${_}this${
          keypath(name, getPropertyAccess)
        }${_}||${_}{}`,
      );
      args.unshift("exports");
    } else {
  ...
  if (hasExports) {
    if (name && !(extend && namedExportsMode)) {
      wrapperIntro = (useVariableAssignment
        ? `${cnst} ${name}`
        : `this${keypath(name, getPropertyAccess)}`) +
        `${_}=${_}${wrapperIntro}`;
    }
    if (isNamespaced) {
      wrapperIntro = setupNamespace(name, "this", globals, snippets, compact) +
        wrapperIntro;
    }
  }
...
function system(
  ...
  let wrapperStart = `System.register(${registeredName}[` +
    ...
    `],${_}(${
      getNonArrowFunctionIntro(wrapperParams, { isAsync: false, name: null })
  ...
  return magicString.indent(`${t}${t}${t}`).append(wrapperEnd).prepend(
    wrapperStart,
  );
}
function analyzeDependencies(
  ...
  exports,
  t,
  { _, cnst, getObject, getPropertyAccess, n },
) {
    ...
    if (imports) {
      for (const specifier of imports) {
        ...
        if (specifier.imported === "*") {
          ...
          setter.push(
            `${specifier.local}${_}=${_}module${
              getPropertyAccess(specifier.imported)
            };`,
          );
        }
      }
      ...
      for (const { imported, reexported } of reexports) {
        if (reexported === "*") {
          ...
          reexportedNames.push([
            ...
            imported === "*"
              ? "module"
              : `module${getPropertyAccess(imported)}`,
          ]);
        }
      }
...
const getStarExcludesBlock = (starExcludes, t, { _, cnst, getObject, n }) =>
  starExcludes
    ? `${n}${t}${cnst} _starExcludes${_}=${_}${
      getObject([...starExcludes].map((prop) => [prop, "1"]), {
        lineBreakIndent: { base: t, t },
      })
    };`
    : "";
const getImportBindingsBlock = (
  ...
  t,
  { _, n },
) => (importBindings.length
  ? `${n}${t}var ${importBindings.join(`,${_}`)};`
  : "");
const getHoistedExportsBlock = (exports, t, snippets) =>
  getExportsBlock(
    exports.filter((expt) => expt.hoisted).map((expt) => ({
      name: expt.exported,
      value: expt.local,
    })),
    t,
...
const getSyntheticExportsBlock = (exports, t, snippets) =>
  getExportsBlock(
    exports
...
const getMissingExportsBlock = (exports, t, snippets) =>
  getExportsBlock(
    exports
...
function umd(
  ...
  if (namedExportsMode && (hasExports || noConflict)) {
    ...
    globalDeps.unshift(
      assignToDeepVariable(
        ...
        `${
          extend
            ? `${globalProp(name, globalVar, getPropertyAccess)}${_}||${_}`
            : ""
        }{}`,
        snippets,
      ),
  ...
  const cjsExport = !namedExportsMode && hasExports
    ? `module.exports${_}=${_}`
    : ``;
  ...
  if (noConflict) {
    ...
    let factory;
    if (!namedExportsMode && hasExports) {
      factory = `${cnst} ${noConflictExportsVar}${_}=${_}${
        assignToDeepVariable(
          name,
          globalVar,
          globals,
      ...
      factory = `${cnst} ${noConflictExportsVar}${_}=${_}${module};${n}` +
        `${t}${t}${factoryVar}(${
          [noConflictExportsVar].concat(globalDeps).join(`,${_}`)
        });`;
    }
    iifeExport =
      `(${getFunctionIntro([], { isAsync: false, name: null })}{${n}` +
      `${t}${t}${cnst} current${_}=${_}${
        safeAccess(name, globalVar, snippets)
      };${n}` +
      `${t}${t}${factory}${n}` +
      `${t}${t}${noConflictExportsVar}.noConflict${_}=${_}${
        getFunctionIntro([], {
          isAsync: false,
          name: null,
        })
      ...
      `${
        globalProp(name, globalVar, getPropertyAccess)
  ...
  const wrapperIntro =
    ...
    // cf. https://v8.dev/blog/preparser#pife
    `})(${globalArg}(${
      getNonArrowFunctionIntro(factoryParams, {
        isAsync: false,
        name: null,
      })
    }{${useStrict}${n}`;
  ...
  magicString.append(`${exportBlock}${namespaceMarkers}${outro}`);
  return magicString.trim().indent(t).append(wrapperOutro).prepend(
    wrapperIntro,
  );
}
...
function getCollapsedSourcemap(
  ...
  if (!originalSourcemap) {
    ...
    const baseSources = sources.map((source, i) =>
      new Source(
        require$$0.resolve(directory, sourceRoot, source),
        sourcesContent[i],
      )
    );
...
function deconflictImportsEsmOrSystem(
  ...
  for (const dependency of dependenciesToBeDeconflicted.dependencies) {
    if (preserveModules || dependency instanceof ExternalModule) {
      dependency.variableName = getSafeName(
        dependency.suggestedVariableName,
        usedNames,
      );
    }
  }
  for (const variable of imports) {
    ...
    if (
      ...
      (preserveModules || module instanceof ExternalModule)
    ) {
      variable.setRenderNames(
        null,
        (module instanceof ExternalModule ? module : chunkByModule.get(module))
          .variableName,
      );
      ...
      variable.setRenderNames(
        ...
        getSafeName(
          [...module.exportedVariables].some((
              [exportedVariable, exportedName],
            ) => exportedName === "*" && exportedVariable.included
            )
  ...
  for (const externalModule of deconflictedDefault) {
    if (
      ...
      canDefaultBeTakenFromNamespace(
        String(interop(externalModule.id)),
        externalLiveBindings,
      )
    ) {
  ...
  for (const variable of exports) {
    ...
    if (exportsByName.has(exportName)) {
      do {
        ...
        RESERVED_NAMES$1.has(exportName) || exportsByName.has(exportName)
      );
    }
...
function getExportMode(
  ...
  if (exportMode === "default") {
    if (exportKeys.length !== 1 || exportKeys[0] !== "default") {
      return error(
        errIncompatibleExportOptionValue("default", exportKeys, facadeModuleId),
      );
    }
...
function decodedSourcemap(map) {
  ...
  const mappings = typeof map.mappings === "string"
    ? decode(map.mappings)
    : map.mappings;
  return { ...map, mappings };
}
...
function renderChunk(
  { code, options, outputPluginDriver, renderChunk, sourcemapChain },
) {
  ...
  return outputPluginDriver.hookReduceArg0("renderChunk", [
    code,
    renderChunk,
    options,
  ], renderChunkReducer);
...
function getGlobalName(module, globals, hasExports, warn) {
  const globalName = typeof globals === "function"
    ? globals(module.id)
    : globals[module.id];
  if (globalName) {
    return globalName;
  ...
  if (hasExports) {
    warn({
      ...
      message:
        `No name was provided for external module '${module.id}' in output.globals – guessing '${module.variableName}'`,
      source: module.id,
    });
...
class Chunk {
  constructor(
    ...
    this.execIndex = orderedModules.length > 0
      ? orderedModules[0].execIndex
      : Infinity;
    ...
    for (const module of orderedModules) {
      ...
      for (const importer of module.includedDynamicImporters) {
        if (!chunkModules.has(importer)) {
          ...
          if (
            module.info.syntheticNamedExports && !outputOptions.preserveModules
          ) {
            includedNamespaces.add(module);
  ...
  generateExports() {
    ...
    if (
      ...
      const exportNamesByVariable = this.facadeModule
        .getExportNamesByVariable();
  ...
  generateFacades() {
    ...
    for (const module of entryModules) {
      if (module.preserveSignature) {
        for (
          const exportedVariable of module.getExportNamesByVariable().keys()
        ) {
    ...
    for (const module of entryModules) {
      const requiredFacades = Array.from(
        new Set(
          module.chunkNames.filter(({ isUserDefined }) => isUserDefined).map((
            { name },
          ) => name),
        ),
        (name) => ({
    ...
    for (const module of this.dynamicEntryModules) {
      ...
      if (
        !this.facadeModule && this.canModuleBeFacade(module, exposedVariables)
      ) {
        this.facadeModule = module;
        this.facadeChunkByModule.set(module, this);
        ...
        !((_a = this.facadeChunkByModule.get(module)) === null || _a === void 0
          ? void 0
          : _a.strictFacade)
      ) {
  ...
  generateIdPreserveModules(
    ...
    const sanitizedId = this.outputOptions.sanitizeFileName(
      id.split(QUERY_HASH_REGEX, 1)[0],
    );
    let path;
    const patternOpt = unsetOptions.has("entryFileNames")
      ? "[name][assetExtname].js"
      : options.entryFileNames;
    const pattern = typeof patternOpt === "function"
      ? patternOpt(this.getChunkInfo())
      : patternOpt;
    if (isAbsolute(sanitizedId)) {
      ...
      const fileName = renderNamePattern(pattern, "output.entryFileNames", {
        assetExtname: () => (NON_ASSET_EXTENSIONS.includes(extension)
          ? ""
          : extension),
  ...
  getChunkName() {
    var _a;
    return ((_a = this.name) !== null && _a !== void 0
      ? _a
  ...
  getExportNames() {
    var _a;
    return ((_a = this.sortedExportNames) !== null && _a !== void 0
      ? _a
    ...
    hash.update(
      this.getExportNames()
        .map((exportName) => {
          ...
          return `${
            relativeId(variable.module.id).replace(/\\/g, "/")
          }:${variable.name}:${exportName}`;
        })
        .join(","),
    );
  ...
  preRender(options, inputBase, snippets) {
    ...
    for (const module of this.orderedModules) {
      ...
      renderedModules[module.id] = {
        get code() {
          var _a, _b;
          return (_b =
                  (_a = renderedModuleSources.get(module)) === null ||
                    _a === void 0
                    ? void 0
    ...
    this.renderedHash = undefined;
    if (
      this.isEmpty && this.getExportNames().length === 0 &&
    ...
    this.renderedExports = this.exportMode === "none"
      ? []
  ...
  async render(options, addons, outputChunk, snippets) {
    ...
    for (const module of this.orderedModules) {
      ...
      const accessedGlobalVariables = this.accessedGlobalsByScope.get(
        module.scope,
      );
    ...
    if (usesTopLevelAwait && format !== "es" && format !== "system") {
      return error({
        ...
        message:
          `Module format ${format} does not support top-level await. Use the "es" or "system" output formats rather.`,
      });
    }
    /* istanbul ignore next */
    if (!this.id) {
  ...
  checkCircularDependencyImport(variable, importingModule) {
    ...
    if (variableModule instanceof Module) {
      ...
      do {
        alternativeReexportModule = importingModule.alternativeReexportModules
          .get(variable);
        if (alternativeReexportModule) {
          const exportingChunk = this.chunkByModule.get(
            alternativeReexportModule,
          );
  ...
  computeContentHashWithDependencies(addons, options, existingNames) {
    const hash = createHash();
    hash.update(
      [addons.intro, addons.outro, addons.banner, addons.footer].join(":"),
    );
  ...
  ensureReexportsAreAvailableForModule(module) {
    ...
    for (const exportedVariable of map.keys()) {
      const isSynthetic = exportedVariable instanceof
  ...
  getChunkDependencyDeclarations(options, getPropertyAccess) {
    ...
    for (const dep of this.dependencies) {
      ...
      const namedExportsMode = dep instanceof ExternalModule ||
  ...
  getDependenciesToBeDeconflicted(
    ...
    addDependenciesWithoutBindings,
    interop,
  ) {
    const dependencies = new Set();
    ...
    for (
      const variable of [...this.exportNamesByVariable.keys(), ...this.imports]
    ) {
        ...
        if (module instanceof ExternalModule) {
          ...
          if (addNonNamespacesAndInteropHelpers) {
            if (variable.name === "default") {
              if (
                defaultInteropHelpersByInteropType[String(interop(module.id))]
              ) {
    ...
    for (let exportName of this.getExportNames()) {
      ...
      if (exportName[0] === "*") {
        ...
        if (module instanceof Module) {
          ...
          needsLiveBinding = externalLiveBindings &&
            (imported !== "default" ||
              isDefaultAProperty(String(interop(module.id)), true));
        }
      }
  ...
  getRelativePath(targetPath, stripJsExtension) {
    let relativePath = normalize(
      relative(require$$0.dirname(this.id), targetPath),
    );
          ...
          if (resolution instanceof Module) {
            ...
            if (chunk === this) {
              ...
              node.setExternalResolution(
                ((_a = this.facadeChunkByModule.get(resolution)) === null ||
                    _a === void 0
                  ? void 0
                  : _a.exportMode) || chunk.exportMode,
                resolution,
                this.outputOptions,
                snippets,
                this.pluginDriver,
  ...
  setExternalRenderPaths(options, inputBase) {
    for (
      const dependency of [...this.dependencies, ...this.dynamicDependencies]
    ) {
      if (dependency instanceof ExternalModule) {
  ...
  setIdentifierRenderResolutions({ format, interop, namespaceToStringTag }) {
    ...
    switch (format) {
      ...
      case "es":
        break;
      case "cjs":
        usedNames.add("module").add("require").add("__filename").add(
          "__dirname",
        );
      // fallthrough
      default:
  ...
  setUpChunkImportsAndExportsForModule(module) {
    ...
    if (
      ...
      module.includedDynamicImporters.some((importer) =>
        this.chunkByModule.get(importer) !== this
      )
    ) {
...
function getChunkNameFromModule(module) {
  var _a, _b, _c, _d;
  return ((_d = (_b = (_a = module.chunkNames.find(({ isUserDefined }) =>
                      isUserDefined
                    )) === null || _a === void 0
                ? void 0
                : _a.name) !== null && _b !== void 0
          ? _b
          : (_c = module.chunkNames[0]) === null || _c === void 0
          ? void 0
    ...
    renderNamePattern(
      ...
      "output.assetFileNames",
      {
        ...
        name: () =>
          emittedName.substr(
            0,
            emittedName.length - require$$0.extname(emittedName).length,
          ),
      },
    ),
    bundle,
...
function getValidSource(source, emittedFile, fileReferenceId) {
  if (!(typeof source === "string" || source instanceof Uint8Array)) {
    const assetName = emittedFile.fileName || emittedFile.name ||
      fileReferenceId;
    return error(
      errFailedValidation(
...
function getChunkFileName(file, facadeChunkByModule) {
  ...
  const fileName = file.fileName ||
    (file.module &&
      ((_a = facadeChunkByModule === null || facadeChunkByModule === void 0
              ? void 0
...
class FileEmitter {
  constructor(graph, options, baseFileEmitter) {
    ...
    this.assertAssetsFinalized = () => {
      for (
        const [referenceId, emittedFile] of this.filesByReferenceId.entries()
      ) {
        if (
          emittedFile.type === "asset" &&
    ...
    this.setOutputBundle = (
      ...
      for (
        const [referenceId, consumedFile] of this.filesByReferenceId.entries()
      ) {
        if (
          consumedFile.type === "asset" && consumedFile.source !== undefined
        ) {
          this.finalizeAsset(
            consumedFile,
            consumedFile.source,
...
function findExistingAssetFileNameWithSource(bundle, source) {
  for (const [fileName, outputFile] of Object.entries(bundle)) {
    if (
      outputFile.type === "asset" && areSourcesEqual(source, outputFile.source)
    ) {
      return fileName;
    }
  }
...
async function createAddons(options, outputPluginDriver) {
  try {
    ...
    return error({
      code: "ADDON_ERROR",
      message:
        `Could not retrieve ${err.hook}. Check configuration of plugin ${err.plugin}.
...
function getChunkAssignments(entryModules, manualChunkAliasByEntry) {
  ...
  const dynamicallyDependentEntryPointsByDynamicEntry =
    getDynamicDependentEntryPoints(
  ...
  function assignEntryToStaticDependencies(entry, dynamicDependentEntryPoints) {
    ...
    for (const module of modulesToHandle) {
      const assignedEntryPoints = getOrCreate(
...
function analyzeModuleGraph(entryModules) {
  ...
  for (const currentEntry of entriesToHandle) {
    ...
    for (const module of modulesToHandle) {
      getOrCreate(dependentEntryPointsByModule, module, () => new Set()).add(
        currentEntry,
      );
      for (const dependency of module.getDependenciesToBeIncluded()) {
        if (!(dependency instanceof ExternalModule)) {
...
function getGenerateCodeSnippets(
  ...
  const getNonArrowFunctionIntro = (params, { isAsync, name }) =>
    `${isAsync ? `async ` : ""}function${name ? ` ${name}` : ""}${_}(${
      params.join(`,${_}`)
    })${_}`;
  ...
  const getDirectReturnFunction = (
    ...
    arrowFunctions
      ? `${name ? ";" : ""}${
        lineBreakIndent ? `${n}${lineBreakIndent.base}` : ""
      }`
  ...
  return {
    ...
    getDirectReturnIifeLeft: (
      ...
      return `${
        wrapIfNeeded(
          `${left}${
            wrapIfNeeded(returned, arrowFunctions && needsArrowReturnParens)
          }${right}`,
...
class Bundle {
  ...
  async addFinalizedChunksToBundle(
    chunks,
    inputBase,
    addons,
    outputBundle,
  ...
  async generateChunks() {
    ...
    for (
      const { alias, modules } of this.outputOptions.inlineDynamicImports
...
function getIncludedModules(modulesById) {
  return [...modulesById.values()].filter((module) =>
    module instanceof Module &&
    (module.isIncluded() || module.info.isEntry ||
...
async function resolveId(
  ...
  return addJsExtensionIfNecessary(
    importer
      ? require$$0.resolve(require$$0.dirname(importer), source)
...
async function addJsExtensionIfNecessary(file, preserveSymlinks) {
  var _a, _b;
  return ((_b =
          (_a = await findFile(file, preserveSymlinks)) !== null &&
            _a !== void 0
            ? _a
            : (await findFile(file + ".mjs", preserveSymlinks))) !== null &&
      _b !== void 0
    ? _b
  ...
  try {
    ...
    if (!preserveSymlinks && stats.isSymbolicLink()) {
      return await findFile(
        await require$$0$1.promises.realpath(file),
        preserveSymlinks,
      );
    }
    if ((preserveSymlinks && stats.isSymbolicLink()) || stats.isFile()) {
      ...
      const files = await require$$0$1.promises.readdir(
        require$$0.dirname(file),
      );
    ...
    for (const plugin of plugins) {
      if (deprecated in plugin) {
        warnDeprecation(
          {
            message:
              `The "${deprecated}" hook used by plugin ${plugin.name} is deprecated. The "${replacement}" hook should be used instead.`,
            plugin: plugin.name,
          },
          active,
          options,
...
async function transform(source, module, pluginDriver, warn) {
  ...
  let originalSourcemap = source.map === null
    ? null
    : decodedSourcemap(source.map);
...
class ModuleLoader {
  ...
  async addAdditionalModules(unresolvedModules) {
    const result = this.extendLoadModulesPromise(
      Promise.all(unresolvedModules.map((id) =>
  ...
  async addEntryModules(unresolvedEntryModules, isUserDefined) {
    ...
    const newEntryModules = await this.extendLoadModulesPromise(
      Promise.all(unresolvedEntryModules.map(({ id, importer }) =>
        ...
        for (let index = 0; index < entryModules.length; index++) {
          ...
          const existingIndexedModule = this.indexedEntryModules.find(
            (indexedModule) =>
              indexedModule.module === entryModule
          );
    ...
    try {
      source = await this.readQueue.run(async () => {
        ...
        return (_a = await this.pluginDriver.hookFirst("load", [id])) !==
              null && _a !== void 0
          ? _a
          : (await require$$0$1.promises.readFile(id, "utf8"));
      });
    } catch (err) {
      ...
      resolveDynamicImportPromises.map((resolveDynamicImportPromise) =>
        resolveDynamicImportPromise.then(
          async ([dynamicImport, resolvedId]) => {
            ...
            return (dynamicImport.resolution = await this
              .fetchResolvedDependency(
  ...
  async fetchStaticDependencies(module, resolveStaticDependencyPromises) {
    for (
      const dependency of await Promise.all(
  ...
  getNormalizedResolvedIdWithoutDefaults(resolveIdResult, importer, source) {
    ...
    const id = makeAbsoluteExternalsRelative
      ...
      : source;
    if (
      resolveIdResult !== false && !this.options.external(id, importer, true)
    ) {
      return null;
    }
    return {
  ...
  getResolveStaticDependencyPromises(module) {
    return Array.from(module.sources, async (source) => [
      source,
      module.resolvedIds[source] = module.resolvedIds[source] ||
  ...
  getResolvedIdWithDefaults(resolvedId) {
    ...
    return {
      ...
      moduleSideEffects:
        (_a = resolvedId.moduleSideEffects) !== null && _a !== void 0
          ? _a
          : this.hasModuleSideEffects(resolvedId.id, !!external),
      syntheticNamedExports:
        (_b = resolvedId.syntheticNamedExports) !== null && _b !== void 0
          ? _b
          : false,
    };
  }
  ...
  async resolveDynamicImport(module, specifier, importer) {
    ...
    if (resolution == null) {
      return (module.resolvedIds[specifier] = module.resolvedIds[specifier] ||
        this.handleResolveId(
...
function addChunkNamesToModule(
  ...
  if (fileName !== null) {
    ...
    let namePosition = 0;
    while (
      ((_a = module.chunkNames[namePosition]) === null || _a === void 0
        ? void 0
  ...
  return ((...args) => {
    if (!deprecationWarningShown) {
      ...
      warnDeprecation(
        {
          message:
            `The "this.${handlerName}" plugin context function used by plugin ${pluginName} is deprecated. The "this.${newHandlerName}" plugin context function should be used instead.`,
          plugin: pluginName,
        },
        activeDeprecation,
        options,
...
function getPluginContext(
  ...
  const context = {
    ...
    emitAsset: getDeprecatedContextHandler(
      (name, source) => fileEmitter.emitFile({ name, source, type: "asset" }),
      "emitAsset",
      "emitFile",
      plugin.name,
      true,
...
function throwInvalidHookError(hookName, pluginName) {
  return error({
    ...
    message:
      `Error running plugin hook ${hookName} for ${pluginName}, expected a function hook.`,
  });
}
class PluginDriver {
...
class Graph {
  constructor(options, watcher) {
    ...
    if (options.cache !== false) {
      ...
      this.pluginCache = ((_b = options.cache) === null || _b === void 0
        ? void 0
    ...
    if (watcher) {
      ...
      const handleClose = () =>
        this.pluginDriver.hookSeqSync("closeWatcher", []);
    ...
    if (this.options.treeshake) {
      ...
      do {
        ...
        if (treeshakingPass === 1) {
          ...
          for (
            const module of [...this.entryModules, ...this.implicitEntryModules]
          ) {
      ...
      for (const importDescription of module.importDescriptions.values()) {
        if (
          ...
          module.warn({
            ...
            message:
              `Non-existent export '${importDescription.name}' is imported from ${
                relativeId(importDescription.module.id)
              }`,
...
function normalizeInputOptions(config) {
  ...
  const context = (_a = config.context) !== null && _a !== void 0
    ? _a
    : "undefined";
  ...
  const options = {
    ...
    experimentalCacheExpiry:
      (_b = config.experimentalCacheExpiry) !== null && _b !== void 0 ? _b : 10,
    external: getIdMatcher(config.external),
    inlineDynamicImports: getInlineDynamicImports$1(
      config,
      onwarn,
      strictDeprecations,
    ),
    ...
    makeAbsoluteExternalsRelative:
      (_c = config.makeAbsoluteExternalsRelative) !== null && _c !== void 0
        ? _c
        : true,
...
const getCache = (config) => {
  var _a;
  return ((_a = config.cache) === null || _a === void 0 ? void 0 : _a.cache) ||
    config.cache;
};
  ...
  return {
    ...
    moduleSideEffects:
      typeof configTreeshake === "object" && configTreeshake.pureExternalModules
        ...
        : getHasModuleSideEffects(
          configWithPreset.moduleSideEffects,
          undefined,
        ),
    propertyReadSideEffects:
...
function normalizeOutputOptions(config, inputOptions, unsetInputOptions) {
  ...
  const outputOptions = {
    amd: getAmd(config),
    assetFileNames: (_a = config.assetFileNames) !== null && _a !== void 0
      ? _a
      : "assets/[name]-[hash][extname]",
    banner: getAddon(config, "banner"),
    chunkFileNames: (_b = config.chunkFileNames) !== null && _b !== void 0
      ? _b
      : "[name]-[hash].js",
    compact,
    ...
    externalLiveBindings:
      (_d = config.externalLiveBindings) !== null && _d !== void 0 ? _d : true,
    file,
    ...
    if (!validatedInteropTypes.has(interop)) {
      ...
      if (typeof interop === "boolean") {
        warnDeprecation(
          {
            message:
              `The boolean value "${interop}" for the "output.interop" option is deprecated. Use ${
...
async function handleGenerateWrite(
  ...
  const { options: outputOptions, outputPluginDriver, unsetOptions } =
...
function getOutputOptions(
  ...
  return normalizeOutputOptions(
    outputPluginDriver.hookReduceArg0Sync("outputOptions", [
      rawOutputOptions.output || rawOutputOptions,
    ], (outputOptions, result) =>
      result || outputOptions, (pluginContext) => {
      const emitError = () =>
...
function createOutput(outputBundle) {
  return {
    output: Object.values(outputBundle).filter((outputFile) =>
...
async function writeOutputFile(outputFile, outputOptions) {
  const fileName = require$$0.resolve(
    outputOptions.dir || require$$0.dirname(outputOptions.file),
    outputFile.fileName,
  );
  // 'recursive: true' does not throw if the folder structure, or parts of it, already exist
  await require$$0$1.promises.mkdir(require$$0.dirname(fileName), {
    recursive: true,
  });
  ...
  if (outputFile.type === "asset") {
    ...
    if (outputOptions.sourcemap && outputFile.map) {
      ...
      if (outputOptions.sourcemap === "inline") {
        ...
        writeSourceMapPromise = require$$0$1.promises.writeFile(
          `${fileName}.map`,
  ...
  return Promise.all([
    require$$0$1.promises.writeFile(fileName, source),

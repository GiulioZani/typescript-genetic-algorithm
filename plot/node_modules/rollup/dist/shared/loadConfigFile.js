...
const isCompatibleTerminal = tty__namespace && tty__namespace.isatty &&
  tty__namespace.isatty(1) && env.TERM && env.TERM !== "dumb";

const isCI = "CI" in env &&
  ("GITHUB_ACTIONS" in env || "GITLAB_CI" in env || "CIRCLECI" in env);

const isColorSupported = !isDisabled &&
...
const filterEmpty = (open, close, replace = open, at = open.length + 1) =>
  (string) =>
...
const createColors = ({ useColor = isColorSupported } = {}) =>
  useColor ? colors : Object.keys(colors).reduce(
...
function handleError(err, recover = false) {
  ...
  if (err.loc) {
    stderr(
      `${
        rollup.relativeId(err.loc.file || err.id)
  ...
  return {
    add: (warning) => {
      ...
      if (warning.code in deferredHandlers) {
        rollup.getOrCreate(deferredWarnings, warning.code, () => []).push(
          warning,
        );
    ...
    flush: () => {
      ...
      const codes = Array.from(deferredWarnings.keys()).sort((a, b) =>
        deferredWarnings.get(b).length - deferredWarnings.get(a).length
      );
  ...
  MISSING_NODE_BUILTINS: (warning) => {
    ...
    stderr(
      `Creating a browser bundle that depends on ${
        rollup.printQuotedStringList(warning.modules)
...
const deferredHandlers = {
  ...
  MIXED_EXPORTS: (warnings) => {
    ...
    const displayedWarnings = warnings.length > 5
      ? warnings.slice(0, 3)
      : warnings;
  ...
  NAMESPACE_CONFLICT(warnings) {
    ...
    for (const warning of warnings) {
      stderr(
        `"${
          bold(rollup.relativeId(warning.reexporter))
        }" re-exports "${warning.name}" from both "${
          rollup.relativeId(warning.sources[0])
        }" and "${rollup.relativeId(warning.sources[1])}" (will be ignored)`,
      );
    }
  },
    ...
    for (const { key: plugin, items } of nestedByPlugin) {
      ...
      for (const { key: message, items } of nestedByMessage) {
        ...
        for (const warning of items) {
          ...
          const id = warning.id ||
            ((_a = warning.loc) === null || _a === void 0 ? void 0 : _a.file);
          if (id) {
  ...
  SOURCEMAP_BROKEN(warnings) {
    ...
    stderr(
      `Plugins that transform code (such as ${
        rollup.printQuotedStringList(plugins)
  ...
  UNRESOLVED_IMPORT(warnings) {
    ...
    for (const warning of warnings) {
      rollup.getOrCreate(dependencies, warning.source, () => []).push(
        warning.importer,
      );
    }
    ...
    for (const warning of warnings) {
      stderr(
        warning.names +
          ...
          warning.source +
          '" but never used in ' +
          rollup.printQuotedStringList(
            warning.sources.map((id) => rollup.relativeId(id)),
          ),
      );
    }
  },
...
function showTruncatedWarnings(warnings) {
  ...
  const displayedByModule = nestedByModule.length > 5
    ? nestedByModule.slice(0, 3)
    : nestedByModule;
  ...
  if (nestedByModule.length > displayedByModule.length) {
    stderr(
      `\n...and ${
        nestedByModule.length - displayedByModule.length
      } other files`,
    );
  }
}
...
async function addPluginsFromCommandOption(commandPlugin, inputOptions) {
  if (commandPlugin) {
    const plugins = Array.isArray(commandPlugin)
      ? commandPlugin
      : [commandPlugin];
...
async function loadAndRegisterPlugin(inputOptions, pluginText) {
  ...
  if (!plugin) {
    throw new Error(
      `Cannot find entry for plugin "${pluginText}". The plugin needs to export a function either as "default" or "${
        getCamelizedPluginBaseName(pluginText)
...
function getCamelizedPluginBaseName(pluginText) {
  var _a;
  return (((_a = pluginText.match(/(@rollup\/plugin-|rollup-plugin-)(.+)$/)) ===
        null || _a === void 0
    ? void 0
    : _a[2]) || pluginText)
    .split(/[\\/]/)
...
async function getConfigList(configFileExport, commandOptions) {
  const config =
    await (typeof configFileExport === "function"

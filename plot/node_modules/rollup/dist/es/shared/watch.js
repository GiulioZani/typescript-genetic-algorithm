...
function mergeOptions(
  config,
  rawCommandOptions = { external: [], globals: undefined },
...
function getCommandOptions(rawCommandOptions) {
  ...
  return {
    ...
    globals: typeof rawCommandOptions.globals === "string"
      ? rawCommandOptions.globals.split(",").reduce(
...
const getOnWarn = (config, defaultOnWarnHandler) =>
  config.onwarn
...
const normalizeObjectOptionValue = (optionValue, objectifyValue) => {
  ...
  if (Array.isArray(optionValue)) {
    return optionValue.reduce(
      (result, value) =>
        value && result && { ...result, ...objectifyValue(value) },
      {},
    );
  }
      ...
      try {
        ...
        if (entryRealPathStats.isDirectory()) {
          ...
          if (
            ...
            full.substr(len, 1) === sysPath$3.sep
          ) {
...
const anymatch$1 = (matchers, testString, options = DEFAULT_OPTIONS) => {
  ...
  const patterns = mtchers
    .filter((item) =>
      typeof item !== "string" ||
      (typeof item === "string" && item.charAt(0) !== BANG$1)
    )
...
function createFsWatchInstance(path, options, listener, errHandler, emitRaw) {
  const handleEvent = (rawEvent, evPath) => {
    ...
    if (evPath && path !== evPath) {
      fsWatchBroadcast(
        sysPath$2.resolve(path, evPath),
        KEY_LISTENERS,
        sysPath$2.join(path, evPath),
      );
    }
  };
  try {
...
function setFSEventsListener(path, realPath, listener, rawEmitter) {
  let watchPath = sysPath$1.extname(realPath)
    ? sysPath$1.dirname(realPath)
    : realPath;
  ...
  for (const watchedPath of FSEventsWatchers.keys()) {
    if (
      realPath.indexOf(sysPath$1.resolve(watchedPath) + sysPath$1.sep) === 0
    ) {
...
class FsEventsHandler$1 {
  ...
  async _addToFsEvents(path, transform, forceAdd, priorDepth) {
    ...
    const opts = this.fsw.options;
    const processPath = typeof transform === FUNCTION_TYPE$1
      ? transform
      : IDENTITY_FN;
      ...
      if (stats.isDirectory()) {
        ...
        this.fsw._readdirp(wh.watchPath, {
          ...
          if (wh.followSymlinks && entry.stats.isSymbolicLink()) {
            ...
            const curDepth = opts.depth === undefined
              ? undefined
              : calcDepth(joinedPath, sysPath$1.resolve(wh.watchPath)) + 1;
...
class FileWatcher {
  ...
  watch(id, isTransformDependency) {
    ...
    if (isTransformDependency) {
      const watcher =
        (_a = this.transformWatchers.get(id)) !== null && _a !== void 0
          ? _a

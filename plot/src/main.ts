...
const algorithmsWithColor = [['pso', 'pink'], ['ga', '#90EE90'], ['gd', 'purple']]
for (const [alg, color] of algorithmsWithColor){
  document.getElementById(alg)!.style.backgroundColor = color
}
window.plot = async () => {
  globalIsDone = false
  const algorithms = algorithmsWithColor.filter(a=>(document.getElementById(a[0])! as HTMLInputElement).checked)
  if (algorithms.length > 0){
    ...
    const plotters = [
      await getPlotter(
        algorithms.map(a=>[`./data/${objectiveFunctionName}_${a[0]}.json`, a[1]]),
        "plot1",

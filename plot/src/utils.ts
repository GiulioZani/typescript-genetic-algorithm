...
const getMinPositionX = (data: Data) =>
  Math.min(...data.flatMap((x) => x.map((y) => y.position[0])));
const getMinPositionY = (data: Data) =>
  Math.min(...data.flatMap((x) => x.map((y) => y.position[1])));
const getMaxPositionX = (data: Data) =>
  Math.max(...data.flatMap((x) => x.map((y) => y.position[0])));
const getMaxPositionY = (data: Data) =>
  Math.max(...data.flatMap((x) => x.map((y) => y.position[1])));
...
const getPlotter = async (
  ...
  const margin = 0.0;
  const maxPositionX =
    Math.max(...allData.map((data) => getMaxPositionX(data))) + margin;
  const maxPositionY =
    Math.max(...allData.map((data) => getMaxPositionY(data))) + margin;
  const minPositionX =
    Math.min(...allData.map((data) => getMinPositionX(data))) - margin;
  const minPositionY =
    Math.min(...allData.map((data) => getMinPositionY(data))) - margin;
